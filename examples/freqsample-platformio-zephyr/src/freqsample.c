/**
 * A frequency counter with a fixed gate period, triggered by another input.
 * (A misinterpretation of how frequency counters work.)
 *
 * Samples taken from sw1, trigger on sw0. Reports frequency to serial console.
 */
...
struct ssm_act *enter_freq_sample(struct ssm_act *caller, ssm_priority_t priority,
                           ssm_depth_t depth, ssm_event_t *gate,
                           ssm_event_t *signal);

void step_freq_sample(struct ssm_act *actg);
...
void step_main(struct ssm_act *actg) {
  ...
  case 0:;
    ...
    ssm_activate(enter_freq_sample(actg, actg->priority + 1 * (1 << actg->depth - 1),
...
struct ssm_act *enter_freq_sample(struct ssm_act *caller, ssm_priority_t priority,
                           ...
                           ssm_event_t *signal) {
  struct ssm_act *actg =
      ssm_enter(sizeof(act_freq_sample_t), step_freq_sample, caller, priority, depth);
  act_freq_sample_t *acts = container_of(actg, act_freq_sample_t, act);
...
void step_freq_sample(struct ssm_act *actg) {
  ...
  case 0:; // Gate not active
    SSM_DEBUG_PRINT("Starting freq_sampleer with period: %llu\r\n", GATE_PERIOD);
  ...
  ssm_leave(actg, sizeof(act_freq_sample_t));
}
